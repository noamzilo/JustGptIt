...
def test_url_shortener_hash_collision():
    ...
    UrlShortener.hash_url = lambda x: 'fixedhash'

    try:
        # Create first URL hash
        url_hash1 = UrlShortener.create_hash(long_url1)
        assert url_hash1 == 'fixedhash'

        # Attempt to create second URL hash (should raise IntegrityError)
        with pytest.raises(IntegrityError):
            UrlShortener.create_hash(long_url2)
    finally:
...
def test_redirect_non_existent_short_url():
    ...
    response = client.get(f'/redirect/{non_existent_hash}')
